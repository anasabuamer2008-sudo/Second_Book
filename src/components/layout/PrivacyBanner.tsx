  ...
  return (
    <div className="fixed bottom-20 md:bottom-4 inset-x-0 z-[60] px-4">
      <div
        ...
        className="max-w-xl mx-auto bg-surface text-foreground rounded-2xl shadow-2xl border border-border p-5 fade-up"
      >
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 shrink-0 bg-accent/15 rounded-xl flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-text-secondary leading-relaxed">{dict.text}</p>
          </div>
        </div>
        <div className="flex flex-col sm:flex-row gap-2 mt-4">
          ...
          <Link
            href={`/${lang}/privacy`}
            onClick={accept}
            className="flex-1 py-2.5 px-4 bg-surface-alt border border-border text-foreground text-sm font-semibold rounded-xl hover:border-primary/40 transition-colors text-center"
          >
            {dict.readMore}
          </Link>
        </div>
