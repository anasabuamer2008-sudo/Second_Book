  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <button
          onClick={handleClear}
          className="inline-flex items-center gap-2 px-4 py-2.5 bg-red-50 text-red-600 text-sm font-bold rounded-xl hover:bg-red-100 transition-colors cursor-pointer"
        >
      ...
      <div className="space-y-6">
        {orders.map((order) => (
          <div key={order.id} className="bg-surface rounded-2xl border border-border p-6">
            {/* Order Header */}
            <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-text-secondary">
                  <p>{dict.orders.date}: {new Date(order.createdAt).toLocaleDateString(lang === "ar" ? "ar-SA" : "he-IL")}</p>
                  <p>{dict.orders.status}: {dict.orders.statusPending}</p>
                </div>
              </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
            ...
            {order.delivery && (
              <div className="mb-5 flex flex-wrap items-center gap-3">
                ...
                <span className="text-sm text-text-secondary">
                  {dict.orders.deliveryFee}: {order.deliveryFee ? `${order.deliveryFee} ${currency}` : dict.orders.free}
                </span>
              </div>
            )}
            ...
            {typeof order.subtotal === "number" && (
              <div className="mb-5 flex justify-end gap-6 text-sm text-text-secondary">
                <span>
                  {dict.orders.subtotal}: <strong className="text-foreground">{order.subtotal} {currency}</strong>
                </span>
                <span>
                  {dict.orders.deliveryFee}: <strong className="text-foreground">{order.deliveryFee ? `${order.deliveryFee} ${currency}` : dict.orders.free}</strong>
                </span>
              </div>
            )}
            ...
            <div className="divide-y divide-border">
              ...
              {order.items.map((book) => (
                <div key={book.id} className="flex items-center gap-3 py-2">
                  <div className="w-8 h-10 bg-gradient-to-br from-primary to-primary-light rounded flex items-center justify-center shrink-0">
                    <span className="text-white font-bold text-xs">{book.title.charAt(0)}</span>
                  </div>
